...
await write(0,"create_pool",[id,"Studionet two-person lifecycle check","Automated sandbox integration test; no real-world commitment.",
  ...
  "self_attested",1000n,1,2,2,1200,3600]);
...
while(Date.now()<Number(pool.join_deadline)*1000+2000) await new Promise(resolve=>setTimeout(resolve,10000));
await write(0,"activate_pool",[id]);
assert.equal((await read("get_pool",[id])).status,"active");
for(let index=0;index<2;index++) {
  for(let attempt=0;attempt<3;attempt++) {
    await write(index,"submit_checkin",[id,"For this sandbox verification test: completed the focus block.","","",crypto.randomUUID()]);
    const participant=await read("get_participant",[id,accounts[index].address]);
    if(participant.status==="success") break;
    assert.equal(participant.status,"active","A definitive fail is not silently retried");
  }
  assert.equal((await read("get_participant",[id,accounts[index].address])).status,"success");
}
...
for(let index=0;index<2;index++) {
  assert.equal((await read("get_credit",[accounts[index].address])).credit_wei,"1000");
