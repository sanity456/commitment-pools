"""Direct-mode safety and lifecycle tests for CommitmentPoolV3.
...
CONTRACT_PATH = str(
    Path(__file__).resolve().parents[1] / "contracts" / "commitment_pool_v3.py"
)
...
@pytest.fixture()
def pool_v3(direct_deploy, direct_vm, direct_alice):
...
def create_pool(pool_v3, direct_vm, creator, pool_id="pool-1", **overrides):
    values = {
        "pool_id": pool_id,
        "title": TITLE,
    ...
    return pool_v3.create_pool(**values)


def join(pool_v3, direct_vm, player, pool_id="pool-1", stake=STAKE):
    direct_vm.sender = player
    direct_vm.value = stake
    try:
        return pool_v3.join(pool_id)
    finally:
        direct_vm.value = 0


def activate(pool_v3, direct_vm, sender, pool_id="pool-1"):
    direct_vm.warp(T_ACTIVE)
    direct_vm.sender = sender
    return pool_v3.activate_pool(pool_id)
...
def submit(pool_v3, direct_vm, player, nonce, pool_id="pool-1", proof="Done"):
    direct_vm.sender = player
    return pool_v3.submit_checkin(pool_id, proof, "", "", nonce)


def test_create_pool_snapshots_terms_and_fee(pool_v3, direct_vm, direct_alice):
    created = create_pool(pool_v3, direct_vm, direct_alice)
    record = pool_v3.get_pool("pool-1")
    ...
    assert pool_v3.can_join("pool-1") is True


def test_create_pool_rejects_unsafe_terms(pool_v3, direct_vm, direct_alice):
    with direct_vm.expect_revert("2 <= min <= max"):
        create_pool(pool_v3, direct_vm, direct_alice, min_players=1)
    with direct_vm.expect_revert("Rounds must be between"):
        create_pool(pool_v3, direct_vm, direct_alice, rounds_required=0)
    with direct_vm.expect_revert("Verification mode"):
        create_pool(pool_v3, direct_vm, direct_alice, verification_mode="magic")
    with direct_vm.expect_revert("Round window must be between"):
        create_pool(pool_v3, direct_vm, direct_alice, round_window_seconds=60)


def test_fee_change_is_owner_only_delayed_and_future_only(
    pool_v3, direct_vm, direct_alice, direct_bob
):
    create_pool(pool_v3, direct_vm, direct_alice, pool_id="old-terms")

    direct_vm.sender = direct_bob
    with direct_vm.expect_revert("Owner-only"):
        pool_v3.schedule_fee_bps(250)

    direct_vm.sender = direct_alice
    scheduled = pool_v3.schedule_fee_bps(250)
    assert scheduled["pending_fee_bps"] == 250
    with direct_vm.expect_revert("timelocked"):
        pool_v3.apply_scheduled_fee()

    direct_vm.warp(T_ACTIVE)
    applied = pool_v3.apply_scheduled_fee()
    assert applied["fee_bps"] == 250
    create_pool(pool_v3, direct_vm, direct_alice, pool_id="new-terms")

    assert pool_v3.get_pool("old-terms")["fee_bps"] == FEE_BPS
    assert pool_v3.get_pool("new-terms")["fee_bps"] == 250


def test_join_requires_exact_stake_and_formation_activation(
    pool_v3, direct_vm, direct_alice, direct_bob, direct_charlie
):
    create_pool(pool_v3, direct_vm, direct_alice, max_players=2)
    with direct_vm.expect_revert("Exact stake required"):
        join(pool_v3, direct_vm, direct_bob, stake=STAKE + 1)
    ...
    activated = activate(pool_v3, direct_vm, direct_alice)
    assert activated["status"] == "active"
    assert pool_v3.can_join("pool-1") is False


def test_underfilled_pool_refunds_every_stake(
    pool_v3, direct_vm, direct_alice, direct_bob
):
    create_pool(pool_v3, direct_vm, direct_alice, min_players=2)
    join(pool_v3, direct_vm, direct_bob)
    result = activate(pool_v3, direct_vm, direct_alice)
    assert result["status"] == "refunding"

    direct_vm.sender = direct_bob
    refunded = pool_v3.claim_formation_refund("pool-1")
    assert refunded["credit_wei"] == str(STAKE)
    assert refunded["status"] == "cancelled"
    assert pool_v3.get_credit(addr(direct_bob))["credit_wei"] == str(STAKE)
    with direct_vm.expect_revert("not refunding"):
        pool_v3.claim_formation_refund("pool-1")
...
def test_late_activation_refunds_instead_of_forcing_missed_rounds(
    ...
    result = pool_v3.activate_pool("pool-1")
    assert result["status"] == "refunding"
    assert pool_v3.get_pool("pool-1")["activation_failure"] == "activation_grace_elapsed"

    direct_vm.sender = direct_bob
    assert pool_v3.claim_formation_refund("pool-1")["credit_wei"] == str(STAKE)
    direct_vm.sender = direct_charlie
    assert pool_v3.claim_formation_refund("pool-1")["status"] == "cancelled"
...
def test_scheduled_rounds_do_not_allow_early_or_late_recovery(
    ...
    with direct_vm.expect_revert("not active"):
        pool_v3.submit_checkin("pool-1", "Done", "", "", "early")

    activate(pool_v3, direct_vm, direct_alice)
    direct_vm.sender = direct_bob
    first = pool_v3.submit_checkin("pool-1", "Done", "", "", "r1")
    assert first["round"] == 1
    with direct_vm.expect_revert("opens at"):
        pool_v3.submit_checkin("pool-1", "Done again", "", "", "r2-early")

    direct_vm.warp(T_ROUND_TWO)
    second = pool_v3.submit_checkin("pool-1", "Done again", "", "", "r2")
    assert second["participant_status"] == "success"

    direct_vm.sender = direct_charlie
    with direct_vm.expect_revert("Round 1 has closed"):
        pool_v3.submit_checkin("pool-1", "Too late", "", "", "late")
    missed = pool_v3.mark_missed_round("pool-1", addr(direct_charlie))
...
def test_unclear_attempts_are_bounded_and_nonces_are_unique(
    ...
    direct_vm.sender = direct_bob
    first = pool_v3.submit_checkin("pool-1", "Maybe", "", "", "attempt-1")
    assert first["retriable"] is True
    with direct_vm.expect_revert("nonce already used"):
        pool_v3.submit_checkin("pool-1", "Maybe", "", "", "attempt-1")
    pool_v3.submit_checkin("pool-1", "Maybe", "", "", "attempt-2")
    last = pool_v3.submit_checkin("pool-1", "Maybe", "", "", "attempt-3")
    assert last["retriable"] is False
    with direct_vm.expect_revert("attempt limit"):
        pool_v3.submit_checkin("pool-1", "Maybe", "", "", "attempt-4")
...
def test_mixed_result_conserves_value_and_uses_fee_snapshot(
    ...
    mock_verdict(direct_vm, "pass")
    submit(pool_v3, direct_vm, direct_bob, "winner")
    mock_verdict(direct_vm, "fail", reasoning="Submission contradicts the rule")
    submit(pool_v3, direct_vm, direct_charlie, "loser", proof="Skipped")

    settled = pool_v3.settle("pool-1")
    ...
    assert pool_v3.get_credit(addr(direct_bob))["credit_wei"] == "1950"
    assert pool_v3.get_credit(addr(direct_alice))["credit_wei"] == "50"
...
def test_all_fail_refunds_minus_fee_instead_of_owner_windfall(
    ...
    submit(pool_v3, direct_vm, direct_bob, "fail-b")
    submit(pool_v3, direct_vm, direct_charlie, "fail-c")
...
def test_withdraw_records_emission_without_claiming_delivery(
    ...
    assert pool_v3.get_payout(payout["id"])["recipient"] == addr(direct_bob)
    assert pool_v3.get_credit(addr(direct_bob))["credit_wei"] == "0"


def test_lists_are_paginated_and_attempts_are_append_only(
    pool_v3, direct_vm, direct_alice, direct_bob, direct_charlie
):
    create_pool(pool_v3, direct_vm, direct_alice, pool_id="a")
    create_pool(pool_v3, direct_vm, direct_alice, pool_id="b")
    page = pool_v3.list_pools(1, 1)
    assert page["total"] == 2
    assert [item["id"] for item in page["items"]] == ["b"]

    join(pool_v3, direct_vm, direct_bob, pool_id="a")
    join(pool_v3, direct_vm, direct_charlie, pool_id="a")
    activate(pool_v3, direct_vm, direct_alice, pool_id="a")
    mock_verdict(direct_vm, "unclear")
    direct_vm.sender = direct_bob
    pool_v3.submit_checkin("a", "Unsure", "", "", "a-1")
    attempt = pool_v3.get_attempt("a", addr(direct_bob), 1, 1)
...
def test_three_player_dust_conservation(pool_v3, direct_vm, direct_alice, direct_bob, direct_charlie, stake):
    create_pool(pool_v3, direct_vm, direct_alice, stake_wei=stake)
    for player in [direct_alice, direct_bob, direct_charlie]:
        join(pool_v3, direct_vm, player, stake=stake)
    activate(pool_v3, direct_vm, direct_alice)
    mock_verdict(direct_vm, "pass")
    submit(pool_v3, direct_vm, direct_bob, "winner-b")
    submit(pool_v3, direct_vm, direct_charlie, "winner-c")
    mock_verdict(direct_vm, "fail")
    submit(pool_v3, direct_vm, direct_alice, "loser-a")
    settled = pool_v3.settle("pool-1")
    fee = stake * FEE_BPS // 10_000
    share, dust = divmod(stake - fee, 2)
    assert settled["conservation_wei"] == str(stake * 3)
    assert pool_v3.get_credit(addr(direct_bob))["credit_wei"] == str(stake + share)
    assert pool_v3.get_credit(addr(direct_charlie))["credit_wei"] == str(stake + share)
    assert pool_v3.get_credit(addr(direct_alice))["credit_wei"] == str(fee + dust)


def test_maximum_cohort_paginates_without_dropping_participants(pool_v3, direct_vm, direct_alice):
    from genlayer.py.types import Address
    create_pool(pool_v3, direct_vm, direct_alice, stake_wei=1, max_players=100)
    players = [Address((index + 1).to_bytes(20, "big")) for index in range(100)]
    for player in players:
        join(pool_v3, direct_vm, player, stake=1)
    assert pool_v3.get_pool("pool-1")["participant_count"] == 100
    first = pool_v3.list_participants("pool-1", 0, 50)
    second = pool_v3.list_participants("pool-1", 50, 50)
    ...
    with direct_vm.expect_revert("full"):
        join(pool_v3, direct_vm, Address((1001).to_bytes(20, "big")), stake=1)


def test_formation_and_round_deadline_edges_are_exclusive(pool_v3, direct_vm, direct_alice, direct_bob, direct_charlie):
    create_pool(pool_v3, direct_vm, direct_alice)
    join(pool_v3, direct_vm, direct_bob)
    join(pool_v3, direct_vm, direct_charlie)
    ...
    with direct_vm.expect_revert():
        submit(pool_v3, direct_vm, direct_bob, "too-late")
    assert pool_v3.get_participant("pool-1", addr(direct_bob))["rounds_passed"] == 0
