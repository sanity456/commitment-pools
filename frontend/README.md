...
Independent Studionet product. Test GEN only; evidence is public. Commit to immutable cohort rules, check in, settle and verify payout delivery.
...
The updated app deliberately blocks new commitments against legacy or mismatched contracts. Run `node scripts/verify-security-release.mjs --expected-fee-bps 500` to verify this release. This read-only command checks successful finalized execution, exact local source bytes, the expected owner and fee, and the correct helper link. It passes for the new v3 pair and fails for legacy code.
...
Source is maintained in the separate private `sanity456/commitment-pools` repository. A push does not automatically redeploy Vercel or Sites. See [wallet authentication](docs/WALLET_AUTH.md) and [Vercel hosting](docs/VERCEL.md).
